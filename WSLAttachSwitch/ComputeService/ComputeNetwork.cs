...
internal class ComputeNetwork : SafeHandle
{
    ...
    private static extern void HcnOpenNetwork(in Guid id, out ComputeNetwork network,
        [MarshalAs(UnmanagedType.LPWStr)] out string err);

    [DllImport("computenetwork.dll", ExactSpelling = true, PreserveSig = false, CharSet = CharSet.Unicode)]
    private static extern void HcnEnumerateNetworks(string query, [MarshalAs(UnmanagedType.LPWStr)] out string networks,
        [MarshalAs(UnmanagedType.LPWStr)] out string err);

    [DllImport("computenetwork.dll", ExactSpelling = true, PreserveSig = false, CharSet = CharSet.Unicode)]
    private static extern void HcnQueryNetworkProperties(ComputeNetwork network, string query,
